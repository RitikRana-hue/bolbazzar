...
router.get('/dashboard', async (req, res) => {
    try {
        const stats = await Promise.all([
            // Total users
            (0, db_1.query)('SELECT COUNT(*) as total FROM users WHERE isActive = true'),
            ...
            `),
            // Total products
            (0, db_1.query)('SELECT COUNT(*) as total FROM products WHERE isActive = true'),
            // Active auctions
            (0, db_1.query)('SELECT COUNT(*) as total FROM auctions WHERE status = $1', ['ACTIVE']),
            // Total orders
            (0, db_1.query)('SELECT COUNT(*) as total FROM orders'),
            // Orders this month
            (0, db_1.query)(`
                SELECT COUNT(*) as total 
                FROM orders 
            ...
            `),
            // Pending disputes
            (0, db_1.query)('SELECT COUNT(*) as total FROM disputes WHERE status = $1', ['OPEN']),
            // Platform wallet balance
            (0, db_1.query)('SELECT COALESCE(SUM(balance), 0) as total FROM wallets'),
        ]);
        const dashboard = {
            users: {
...
router.get('/users', async (req, res) => {
    try {
        const { role, status = 'active', search, page = 1, limit = 20 } = req.query;
...
router.put('/users/:id/status', async (req, res) => {
    try {
        ...
        await (0, db_1.transaction)(async (client) => {
            // Update user status
            await client.query('UPDATE users SET isActive = $1 WHERE id = $2', [isActive, id]);
            // Log admin action
            await client.query(`
...
router.put('/products/:id/status', async (req, res) => {
    try {
        ...
        await (0, db_1.transaction)(async (client) => {
            // Update product status
            await client.query('UPDATE products SET status = $1 WHERE id = $2', [status, id]);
            // Log admin action
            await client.query(`
                INSERT INTO admin_logs (adminId, action, target, targetId, details)
                VALUES ($1, $2, $3, $4, $5)
...
router.get('/logs', async (req, res) => {
    try {
        const { action, target, adminId, page = 1, limit = 50 } = req.query;
        let whereClause = 'WHERE 1=1';
        const params = [];
        let paramCount = 0;
        if (action) {
