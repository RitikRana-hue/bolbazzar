...
router.get('/:orderId', async (req, res) => {
    try {
        ...
        const order = await (0, db_1.query)(`
        ...
        const delivery = await (0, db_1.query)(`
            SELECT 
                d.*,
                o.shipping_address
            FROM deliveries d
        ...
        const timeline = await (0, db_1.query)(`
...
router.put('/:orderId/status', async (req, res) => {
    try {
        ...
        `, [status, orderId]);
        // Add timeline entry
        const delivery = await (0, db_1.query)('SELECT id FROM deliveries WHERE order_id = $1', [orderId]);
        if (delivery.rows.length > 0) {
            await (0, db_1.query)(`
...
router.post('/agent/login', async (req, res) => {
    try {
        ...
        const agent = await (0, db_1.query)(`
...
router.get('/agent/deliveries', async (req, res) => {
    try {
        const agentId = req.headers['agent-id']; // From auth middleware
        const deliveries = await (0, db_1.query)(`
            SELECT 
                d.*,
                o.id as order_id,
                o.total_amount,
...
router.put('/agent/:orderId/update', async (req, res) => {
    try {
        const { orderId } = req.params;
        ...
        await (0, db_1.transaction)(async (client) => {
            ...
            if (status === 'delivered') {
                ...
                if (order.rows.length > 0) {
                    ...
                    await (0, notifications_1.createNotification)(o.buyer_id, 'order', 'Order Delivered', 'Your order has been delivered successfully. Please confirm receipt.', `/delivery/${orderId}`, { orderId, requiresConfirmation: true });
                    // If escrow exists and auto-release is enabled, start countdown
                    if (o.escrow_id) {
                        // Escrow will auto-release after 7 days unless buyer disputes
                        await (0, notifications_1.createNotification)(o.buyer_id, 'system', 'Confirm Delivery', 'Please confirm delivery within 7 days or funds will be automatically released to the seller.', `/delivery/${orderId}`, { orderId, autoReleaseDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) });
                    }
                }
            }
            ...
            if (order.rows.length > 0) {
                await (0, notifications_1.createNotification)(order.rows[0].buyer_id, 'order', `Delivery Update: ${status.replace('_', ' ')}`, description, `/delivery/${orderId}`, { orderId, status, location });
            }
        });
        res.json({ success: true });
    }
...
router.post('/agent/:orderId/photos', async (req, res) => {
    try {
        const { orderId } = req.params;
        ...
        const photoUrls = photos.map((photo, index) => `https://storage.instasell.com/delivery/${orderId}/${photoType}_${index}_${Date.now()}.jpg`);
        await (0, db_1.query)(`
...
router.post('/agent/:orderId/signature', async (req, res) => {
    try {
        const { orderId } = req.params;
...
router.post('/agent/:orderId/open-box', async (req, res) => {
    try {
        const { orderId } = req.params;
...
router.get('/agent/metrics', async (req, res) => {
    try {
        const agentId = req.headers['agent-id'];
        const metrics = await (0, db_1.query)(`
            SELECT 
