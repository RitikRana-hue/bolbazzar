...
router.post('/create-intent', rateLimiting_1.paymentLimiter, (0, validation_1.validateBody)(schemas_1.createPaymentIntentSchema), async (req, res) => {
    try {
        ...
        const idempotencyKey = req.headers['idempotency-key'] ||
            crypto_1.default.createHash('sha256').update(`${userId}-${orderId}-${amount}`).digest('hex');
        const result = await payment_service_1.PaymentService.createPaymentIntent(orderId, amount, currency, userId, idempotencyKey);
        // Audit payment attempt
        await auditLog_1.auditPayment.paymentAttempt(req, orderId, amount, true);
        res.json({
            success: true,
            ...result
        });
        ...
        await auditLog_1.auditPayment.paymentAttempt(req, req.body.orderId, req.body.amount, false, error instanceof Error ? error.message : 'Failed to create payment intent');
        res.status(500).json({
...
router.post('/confirm', rateLimiting_1.paymentLimiter, async (req, res) => {
    try {
        ...
        const idempotencyKey = req.headers['idempotency-key'] ||
            crypto_1.default.createHash('sha256').update(`confirm-${userId}-${paymentIntentId}`).digest('hex');
        const result = await payment_service_1.PaymentService.confirmPayment(paymentIntentId, idempotencyKey);
        res.json(result);
    }
    catch (error) {
...
router.get('/status/:orderId', (0, validation_1.validateParams)(schemas_1.idSchema), async (req, res) => {
    try {
...
router.post('/refund', rateLimiting_1.paymentLimiter, async (req, res) => {
    try {
        ...
        const result = await payment_service_1.PaymentService.processRefund(orderId, amount, reason);
        // Audit refund
        await auditLog_1.auditPayment.refund(req, orderId, amount, true);
        res.json(result);
    }
    catch (error) {
        console.error('Error processing refund:', error);
        // Audit failed refund
        await auditLog_1.auditPayment.refund(req, req.body.orderId, req.body.amount, false, error instanceof Error ? error.message : 'Failed to process refund');
        res.status(500).json({
...
router.post('/escrow/release/:orderId', (0, validation_1.validateParams)(schemas_1.idSchema), async (req, res) => {
    try {
        const { orderId } = req.params;
        ...
        const result = await payment_service_1.PaymentService.releaseEscrow(orderId, reason);
        res.json(result);
    }
    catch (error) {
...
router.get('/wallet/balance', async (req, res) => {
    try {
        const userId = req.user?.id;
        const wallet = await (0, db_1.query)(`
            SELECT 
                availableBalance,
                pendingBalance,
                totalEarned,
...
router.post('/wallet/withdraw', async (req, res) => {
    try {
        ...
        const walletResult = await (0, db_1.query)(`
        ...
        const withdrawalResult = await (0, db_1.query)(`
...
router.get('/transactions', async (req, res) => {
    try {
        ...
        const transactions = await payment_service_1.PaymentService.getTransactionHistory(userId);
...
router.post('/webhook', async (req, res) => {
    try {
        ...
        const event = stripe_service_1.StripeService.constructWebhookEvent(req.body, signature, config_1.default.payment.stripe.webhookSecret);
        // Handle the event
...
router.get('/admin/reconciliation/:date', async (req, res) => {
    try {
        ...
        const report = await reconciliation_service_1.ReconciliationService.getReconciliationReport(date);
        if (!report) {
...
router.post('/admin/reconciliation/:date', async (req, res) => {
    try {
        const { date } = req.params;
        const userId = req.user?.id;
        // Verify user is admin
        ...
        const report = await reconciliation_service_1.ReconciliationService.runDailyReconciliation(date);
        res.json(report);
    }
    catch (error) {
