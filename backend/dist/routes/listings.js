...
router.use('/create', auth_1.authenticateToken);
router.use('/:id/watchlist', auth_1.authenticateToken);
router.post('/', auth_1.authenticateToken);
router.put('/:id', auth_1.authenticateToken);
router.delete('/:id', auth_1.authenticateToken);
// Get all listings with filters
router.get('/', (0, validation_1.validateQuery)(schemas_1.searchSchema), async (req, res) => {
    try {
        const { category, condition, minPrice, maxPrice, location, search, sortBy = 'createdAt', sortOrder = 'DESC', page = 1, limit = 20 } = req.query;
        ...
        res.status(500).json({ error: 'Failed to fetch listings', details: error instanceof Error ? error.message : 'Unknown error' });
    }
});
// Get single listing
router.get('/:id', (0, validation_1.validateParams)(schemas_1.idSchema), async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)(`
            SELECT 
        ...
        // Get product images
        const images = await (0, db_1.query)('SELECT * FROM product_images WHERE productId = $1 ORDER BY isPrimary DESC, sortOrder ASC', [id]);
        // Get recent reviews
        const reviews = await (0, db_1.query)(`
            SELECT 
                r.*,
        ...
        await (0, db_1.query)('UPDATE products SET viewCount = viewCount + 1 WHERE id = $1', [id]);
        res.json({
            ...product,
            images: images.rows,
...
// Create new listing
router.post('/', rateLimiting_1.listingLimiter, (0, validation_1.validateBody)(schemas_1.createListingSchema), async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Authentication required' });
        }
        const { categoryId, title, description, condition, brand, model, price, originalPrice, stock, weight, dimensions, features, tags, images, isAuction, auctionData } = req.body;
        const result = await (0, db_1.transaction)(async (client) => {
            ...
            if (isAuction && auctionData) {
                const { startingPrice, reservePrice, duration, // in hours
                autoExtend = true } = auctionData;
        ...
        await (0, notifications_1.createNotification)(userId, 'listing', 'New Listing Created', `Your listing "${title}" has been created successfully`, `/p/${result.id}`, { productId: result.id, isAuction });
        res.status(201).json({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        // Verify ownership
        const product = await (0, db_1.query)('SELECT sellerId FROM products WHERE id = $1', [id]);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const userId = req.user?.id;
        // Verify ownership
        const product = await (0, db_1.query)('SELECT sellerId, status FROM products WHERE id = $1', [id]);
        if (product.rows.length === 0) {
            return res.status(404).json({ error: 'Product not found' });
        }
        if (product.rows[0].sellerId !== userId) {
        ...
        const activeOrders = await (0, db_1.query)('SELECT id FROM orders WHERE id IN (SELECT orderId FROM order_items WHERE productId = $1) AND status NOT IN ($2, $3, $4)', [id, 'DELIVERED', 'CANCELLED', 'REFUNDED']);
        ...
        await (0, db_1.query)('UPDATE products SET isActive = false, status = $1 WHERE id = $2', ['DELETED', id]);
...
router.get('/seller/:sellerId', async (req, res) => {
    try {
        ...
        // Get total count
        const countResult = await (0, db_1.query)('SELECT COUNT(*) as total FROM products WHERE sellerId = $1 AND status = $2 AND isActive = true', [sellerId, status]);
        const total = parseInt(countResult.rows[0].total);
        const totalPages = Math.ceil(total / parseInt(limit));
        res.json({
            products: result.rows,
...
router.post('/:id/watchlist', async (req, res) => {
    try {
        ...
        const existing = await (0, db_1.query)('SELECT id FROM watchlist WHERE userId = $1 AND productId = $2', [userId, id]);
        if (existing.rows.length > 0) {
            return res.status(409).json({ error: 'Already in watchlist' });
        }
        await (0, db_1.query)('INSERT INTO watchlist (userId, productId) VALUES ($1, $2)', [userId, id]);
        // Update favorite count
        await (0, db_1.query)('UPDATE products SET favoriteCount = favoriteCount + 1 WHERE id = $1', [id]);
...
router.delete('/:id/watchlist', async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        const result = await (0, db_1.query)('DELETE FROM watchlist WHERE userId = $1 AND productId = $2', [userId, id]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: 'Not in watchlist' });
        }
        // Update favorite count
        await (0, db_1.query)('UPDATE products SET favoriteCount = GREATEST(favoriteCount - 1, 0) WHERE id = $1', [id]);
