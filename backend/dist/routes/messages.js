...
const createNotification = async (userId, type, title, message, actionUrl, data) => {
    try {
        await (0, db_1.query)(`
...
router.get('/conversations', async (req, res) => {
    try {
        ...
        const conversations = await (0, db_1.query)(`
            SELECT 
                c.id,
                c.participant_id,
...
router.get('/conversations/:id/messages', async (req, res) => {
    try {
        ...
        const conversation = await (0, db_1.query)('SELECT * FROM conversations WHERE id = $1 AND (user_id = $2 OR participant_id = $2)', [id, userId]);
...
router.post('/conversations/:id/messages', async (req, res) => {
    try {
        ...
        // Insert message
        const message = await (0, db_1.query)(`
...
router.post('/conversations/:id/exchange-contact', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await createNotification(otherUserId, 'message', 'Contact Number Shared', 'The other party has shared their contact number', `/messages?conversation=${id}`, { conversationId: id });
        res.json({ success: true });
    }
    catch (error) {
...
router.get('/conversations/:id/contacts', async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        // Verify access and payment
        const conversation = await (0, db_1.query)(`
        ...
        const contacts = await (0, db_1.query)(`
            SELECT ce.*, u.name 
...
router.post('/conversations/:id/dispute', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Notify other party
        await createNotification(result.otherUserId, 'dispute', 'Dispute Raised', `A dispute has been raised for order #${conversation.rows[0].order_id}`, `/account/disputes`, { orderId: conversation.rows[0].order_id, disputeId: result.disputeId });
