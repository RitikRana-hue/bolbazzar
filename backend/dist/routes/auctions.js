...
const index_1 = require("../index"); // Socket.IO instance
...
router.use('/:id/bid', auth_1.authenticateToken);
router.use('/user/bids', auth_1.authenticateToken);
router.use('/:id/end', auth_1.authenticateToken);
router.use('/:id/watch', auth_1.authenticateToken);
// Get all active auctions
router.get('/', (0, validation_1.validateQuery)(schemas_1.paginationSchema), async (req, res) => {
    try {
        const { category, minPrice, maxPrice, endingSoon, // within 24 hours
        search, sortBy = 'endTime', sortOrder = 'ASC', page = 1, limit = 20 } = req.query;
...
router.get('/:id', (0, validation_1.validateParams)(schemas_1.idSchema), async (req, res) => {
    try {
        ...
        // Get product images
        const images = await (0, db_1.query)('SELECT * FROM product_images WHERE productId = $1 ORDER BY isPrimary DESC, sortOrder ASC', [auction.productId]);
        // Get recent bids
        const bids = await (0, db_1.query)(`
            SELECT 
                ab.*,
...
// Place bid
router.post('/:id/bid', rateLimiting_1.biddingLimiter, (0, validation_1.validateParams)(schemas_1.idSchema), (0, validation_1.validateBody)(schemas_1.placeBidSchema), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const result = await (0, db_1.transaction)(async (client) => {
            ...
            const wallet = await client.query('SELECT availableBalance FROM wallets WHERE userId = $1', [userId]);
            ...
            if (auctionData.autoExtend && timeLeft <= 5 * 60 * 1000) { // 5 minutes
                ...
                await client.query('UPDATE auctions SET endTime = $1 WHERE id = $2', [newEndTime, id]);
            }
        ...
        index_1.io.to(`auction_${id}`).emit('newBid', {
            auctionId: id,
            bidId: result.id,
            amount,
            bidderId: userId,
        ...
        const previousBids = await (0, db_1.query)('SELECT bidderId FROM auction_bids WHERE auctionId = $1 AND id != $2 ORDER BY amount DESC LIMIT 1', [id, result.id]);
        if (previousBids.rows.length > 0) {
            await (0, notifications_1.createNotification)(previousBids.rows[0].bidderId, 'auction', 'Outbid Alert', 'You have been outbid on an auction', `/auctions/${id}`, { auctionId: id, newBid: amount });
        }
        // Audit successful bid
        await auditLog_1.auditBid.placeBid(req, id, amount, true);
        res.json({
        ...
        // Audit failed bid
        await auditLog_1.auditBid.placeBid(req, req.params.id, req.body.amount, false, error instanceof Error ? error.message : 'Failed to place bid');
        res.status(500).json({
    ...
    try {
        ...
        const result = await (0, db_1.transaction)(async (client) => {
            ...
            if (auctionData.winnerId && auctionData.winningAmount) {
                ...
                await client.query('UPDATE products SET status = $1 WHERE id = $2', ['SOLD', auctionData.productId]);
                // Notify winner
                await (0, notifications_1.createNotification)(auctionData.winnerId, 'auction', 'Auction Won!', `Congratulations! You won the auction for "${auctionData.title}"`, `/orders/${order.rows[0].id}`, { auctionId: id, orderId: order.rows[0].id });
                // Notify seller
                await (0, notifications_1.createNotification)(auctionData.sellerId, 'auction', 'Auction Ended', `Your auction for "${auctionData.title}" has ended with a winning bid`, `/account/orders`, { auctionId: id, orderId: order.rows[0].id });
            }
        ...
        // Emit real-time update
        index_1.io.to(`auction_${id}`).emit('auctionEnded', {
            auctionId: id,
