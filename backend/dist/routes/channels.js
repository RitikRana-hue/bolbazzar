...
router.post('/seller/:sellerId', async (req, res) => {
    try {
        ...
        const seller = await (0, db_1.query)('SELECT id, name FROM users WHERE id = $1 AND role = $2', [sellerId, 'seller']);
        if (seller.rows.length === 0) {
            return res.status(404).json({ error: 'Seller not found' });
        }
        const result = await notification_service_1.NotificationService.subscribeToSeller(userId, sellerId, notificationTypes);
        res.json(result);
    }
    catch (error) {
...
router.post('/category/:categoryId', async (req, res) => {
    try {
        ...
        const category = await (0, db_1.query)('SELECT id, name FROM categories WHERE id = $1', [categoryId]);
        if (category.rows.length === 0) {
            return res.status(404).json({ error: 'Category not found' });
        }
        const result = await notification_service_1.NotificationService.subscribeToCategory(userId, categoryId, notificationTypes);
        res.json(result);
    }
    catch (error) {
...
router.post('/keyword', async (req, res) => {
    try {
        ...
        const result = await notification_service_1.NotificationService.subscribeToKeyword(userId, keyword.trim(), notificationTypes);
        res.json(result);
    }
    catch (error) {
...
router.get('/preferences', async (req, res) => {
    try {
        ...
        const preferences = await notification_service_1.NotificationService.getUserNotificationPreferences(userId);
...
router.put('/preferences', async (req, res) => {
    try {
        ...
        const result = await notification_service_1.NotificationService.updateNotificationPreferences(userId, preferences);
        res.json(result);
    }
    catch (error) {
...
router.get('/sellers', async (req, res) => {
    try {
        ...
        const sellers = await (0, db_1.query)(`
            SELECT 
...
router.get('/categories', async (req, res) => {
    try {
        const userId = req.user?.id;
        const categories = await (0, db_1.query)(`
            SELECT 
