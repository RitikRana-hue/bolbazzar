...
router.get('/:userId', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)('SELECT * FROM wallets WHERE user_id = $1', [userId]);
...
router.post('/:userId/topup', async (req, res) => {
    try {
        ...
        await (0, db_1.transaction)(async (client) => {
            ...
            await client.query(`INSERT INTO transactions (id, user_id, amount, type, created_at)
         VALUES ($1, $2, $3, $4, NOW())`, [txnId, userId, amount, 'topup']);
        });
...
router.post('/:userId/use-gas', async (req, res) => {
    try {
        const { userId } = req.params;
        const { amount } = req.body;
        const walletRes = await (0, db_1.query)('SELECT * FROM wallets WHERE user_id = $1', [userId]);
        ...
        await (0, db_1.transaction)(async (client) => {
            // Update wallet
            await client.query(`UPDATE wallets SET gas_balance = gas_balance - $1, updated_at = NOW() WHERE user_id = $2`, [amount, userId]);
            // Create transaction record
            const txnId = (0, uuid_1.v4)();
            await client.query(`INSERT INTO transactions (id, user_id, amount, type, created_at)
         VALUES ($1, $2, $3, $4, NOW())`, [txnId, userId, amount, 'charge']);
        });
...
router.post('/:userId/withdraw', async (req, res) => {
    try {
        ...
        await (0, db_1.transaction)(async (client) => {
            ...
            const txnId = (0, uuid_1.v4)();
            await client.query(`INSERT INTO transactions (id, user_id, amount, type, created_at)
         VALUES ($1, $2, $3, $4, NOW())`, [txnId, userId, amount, 'withdraw']);
        });
