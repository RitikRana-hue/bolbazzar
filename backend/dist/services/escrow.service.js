...
class EscrowService {
     ...
     */
    static async createEscrow(orderId, buyerId, sellerId, amount, paymentIntentId) {
        return await (0, db_1.transaction)(async (client) => {
            ...
            await (0, notifications_1.createNotification)(sellerId, 'payment', 'Payment Received', `Payment of $${amount.toFixed(2)} is being held in escrow for order #${orderId}`, `/account/orders/${orderId}`, { orderId, amount });
     ...
     */
    static async releaseEscrow(escrowId, reason) {
        return await (0, db_1.transaction)(async (client) => {
            // Get escrow details
            ...
            `, [escrow.order_id]);
            // Notify seller
            await (0, notifications_1.createNotification)(escrow.seller_id, 'payment', 'Funds Released', `$${escrow.amount.toFixed(2)} has been released to your account`, `/account/wallet`, { orderId: escrow.order_id, amount: escrow.amount });
            return escrow;
        });
    }
    /**
    ...
    static async autoReleaseExpiredEscrows() {
        const expiredEscrows = await (0, db_1.query)(`
     ...
     */
    static async requestWithdrawal(sellerId, amount, method) {
        return await (0, db_1.transaction)(async (client) => {
    ...
    static async processWithdrawal(withdrawalId, status, adminNotes) {
        return await (0, db_1.transaction)(async (client) => {
            ...
            // Notify seller
            await (0, notifications_1.createNotification)(w.user_id, 'payment', `Withdrawal ${status}`, status === 'approved'
                ? `Your withdrawal of $${w.amount.toFixed(2)} has been processed`
                : `Your withdrawal request was rejected: ${adminNotes}`, `/account/wallet`);
            return withdrawal.rows[0];
        });
    }
    /**
     ...
     */
    static async lockFundsForDispute(orderId, disputeId) {
        return await (0, db_1.transaction)(async (client) => {
            ...
            if (order.rows.length > 0) {
                ...
                await (0, notifications_1.createNotification)(o.seller_id, 'dispute', 'Funds Locked', `Funds for order #${orderId} have been locked due to a dispute`, `/account/disputes`, { orderId, amount: o.amount });
            }
        });
    }
}
