...
class AuctionService {
    ...
    static async handleAutoExtension(auctionId) {
        try {
            ...
            if (timeLeft <= extensionThreshold) {
                ...
                await (0, db_1.query)('UPDATE auctions SET endTime = $1 WHERE id = $2', [newEndTime, auctionId]);
                // Emit real-time update
                index_1.io.to(`auction_${auctionId}`).emit('auctionExtended', {
                    auctionId,
                    newEndTime,
    ...
    static async processAutomaticBid(auctionId, newBidAmount) {
        try {
            // Get all automatic bids for this auction
            const autoBids = await (0, db_1.query)(`
            ...
            if (autoBids.rows.length === 0)
                return;
            const highestAutoBid = autoBids.rows[0];
            // Calculate next bid amount
            const auction = await (0, db_1.query)('SELECT bidIncrement FROM auctions WHERE id = $1', [auctionId]);
            ...
            if (nextBidAmount <= highestAutoBid.maxBid) {
                // Place automatic bid
                await this.placeBid(auctionId, highestAutoBid.bidderId, nextBidAmount, highestAutoBid.maxBid, true);
            }
        }
        catch (error) {
    ...
    static async placeBid(auctionId, bidderId, amount, maxBid, isAutomatic = false) {
        return await (0, db_1.transaction)(async (client) => {
            ...
            if (!isAutomatic) {
                const wallet = await client.query('SELECT availableBalance FROM wallets WHERE userId = $1', [bidderId]);
    ...
    static async endAuction(auctionId) {
        return await (0, db_1.transaction)(async (client) => {
            ...
            if (auctionData.reservePrice && auctionData.winningAmount < auctionData.reservePrice) {
                ...
                `, [auctionId]);
                // Notify seller
                await (0, notifications_1.createNotification)(auctionData.sellerId, 'auction', 'Auction Ended - Reserve Not Met', `Your auction for "${auctionData.title}" ended without meeting the reserve price`, `/account/listings`, { auctionId, reason: 'reserve_not_met' });
                // Release all funds
                await client.query(`
                    UPDATE wallets 
            ...
            if (auctionData.winnerId && auctionData.winningAmount) {
                ...
                // Notify winner
                await (0, notifications_1.createNotification)(auctionData.winnerId, 'auction', 'Congratulations! You Won!', `You won the auction for "${auctionData.title}"`, `/orders/${order.rows[0].id}`, { auctionId, orderId: order.rows[0].id });
                // Notify seller
                await (0, notifications_1.createNotification)(auctionData.sellerId, 'auction', 'Auction Sold!', `Your auction for "${auctionData.title}" sold for $${auctionData.winningAmount}`, `/account/orders`, { auctionId, orderId: order.rows[0].id });
    ...
    static async checkExpiredAuctions() {
        try {
            const expiredAuctions = await (0, db_1.query)(`
    ...
    static async getAuctionAnalytics(auctionId) {
        try {
            const analytics = await (0, db_1.query)(`
                SELECT 
                    a.id,
                    a.startingPrice,
                    a.currentPrice,
            ...
            // Get bid history
            const bidHistory = await (0, db_1.query)(`
                SELECT 
                    ab.amount,
                    ab.createdAt,
    ...
    static async getSuggestedStartingPrice(categoryId, condition) {
        try {
            const result = await (0, db_1.query)(`
