...
export async function processCleanupTasks(job: Job<CleanupTaskJob>): Promise<void> {
...
async function cleanupExpiredTokens(olderThan?: Date, batchSize: number = 1000): Promise<number> {
...
async function cleanupOldLogs(olderThan?: Date, batchSize: number = 1000): Promise<number> {
...
async function cleanupTempFiles(olderThan?: Date, batchSize: number = 1000): Promise<number> {
...
async function cleanupWebhookEvents(olderThan?: Date, batchSize: number = 1000): Promise<number> {
...
export async function scheduleCleanupTask(
    taskType: CleanupTaskJob['taskType'],
    options: {
        olderThan?: Date;
        batchSize?: number;
        delay?: number;
...
export async function scheduleEmergencyCleanup(): Promise<void> {
    const now = new Date();
