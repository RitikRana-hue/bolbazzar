...
export async function processAuctionAutoEnd(job: Job<AuctionAutoEndJob>): Promise<void> {
        ...
        await transaction(async (client) => {
            ...
            if (auction.winnerid && auction.winningamount) {
                ...
                await scheduleNotification(
                    ...
                    'auction_ended',
                    'Auction Ended',
                    `Your auction for "${auction.title}" has ended with a winning bid`,
                    {
                        data: { auctionId, orderId, amount: auction.winningamount },
                        channels: ['push', 'email'],
                        priority: 1
...
export async function scheduleAuctionAutoEnd(auctionId: string, endTime: Date): Promise<void> {
