...
const config: Config = {
    ...
    email: {
        provider: (process.env.EMAIL_PROVIDER as any) || (nodeEnv === 'development' ? 'console' : 'sendgrid'),
    ...
    fileUpload: {
        provider: (process.env.FILE_UPLOAD_PROVIDER as any) || 'local',
    ...
    logging: {
        level: (process.env.LOG_LEVEL as any) || (nodeEnv === 'production' ? 'info' : 'debug'),
