...
export class ReconciliationService {
     ...
     */
    static async runDailyReconciliation(date: string): Promise<ReconciliationReport> {
        try {
            ...
            const stripeTransactions = await StripeService.listBalanceTransactions({
                created: {
     ...
     */
    private static async getDatabaseTotals(date: string): Promise<{
        payouts: number;
        platformFees: number;
        refunds: number;
        disputes: number;
     ...
     */
    private static async storeReconciliationReport(report: ReconciliationReport): Promise<void> {
        try {
            await query(`
     ...
     */
    static async getReconciliationReport(date: string): Promise<ReconciliationReport | null> {
        try {
     ...
     */
    static async autoReconcileYesterday(): Promise<ReconciliationReport> {
        try {
     ...
     */
    static async getPendingReconciliations(daysBack: number = 30): Promise<string[]> {
        try {
            ...
            const reconciledDates = new Set(
                reconciledResult.rows.map((row: any) => row.date)
            );
     ...
     */
    static async reconcilePendingDates(dates: string[]): Promise<ReconciliationReport[]> {
